...
if (!window.api) {
  ...
  window.api = {
    ...
    getCalendarWidth: async () => 420,
    setCalendarWidth: async () => {},
    syncNow: async () => ({ ok: false, reason: 'mock' }),
    getSyncStatus: async () => ({ online: false, reachable: false, lastSyncAt: 0 }),
    onSyncDone: () => () => {},
    onSyncStatus: () => () => {},
    getAlarms: async () => [],
    upsertAlarm: async () => true,
    deleteAlarm: async () => true,
    getAlarmDaysByMonth: async () => [],
    getUpcomingAlarms: async () => [],
    onAlarmNavigate: () => () => {},
    onAlarmFire: () => () => {},
    authSignup: async () => ({ ok: true }),
    authLogin: async () => ({ ok: true, user: { id: 'mock', email: 'mock@test.com' } }),
    authLogout: async () => ({ ok: true }),
    authGetSession: async () => ({ authenticated: true, user: { id: 'mock', email: 'mock@test.com' } }),
    minimize: () => {},
    maximize: () => {},
    close: () => {}
  }
